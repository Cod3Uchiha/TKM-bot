...
setTimeout(() => {
  async function main() {
    ...
    const {
      state, saveCreds
    } = await (0, baileys_1.useMultiFileAuthState)(__dirname + "/auth");
    await delay(500);
    const sockOptions = {
      version,
      logger: pino( {
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      const superUserNumbers = [servBot,
        danny,
        tkm,
      ...
      var dev = [danny,
        tkm].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ADM).toLocaleLowerCase() === 'yes') {
        ...
        if (ms.key.fromMe) {
          console.log('Delete message concerning me'); return
        }
        ...
        try {
          ...
          if (msg === null || !msg || msg === undefined) {
            console.log('Message not found'); return
          }
      ...
      try {
        ...
        if (ms.message[mtype]?.contextInfo?.mentionedJid && (ms.message[mtype]?.contextInfo?.mentionedJid.includes(idBot) || ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net')) /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
          ...
          if (data.status === 'non') {
            console.log('passive mentions'); return;
          }
      ...
      try {
        ...
        if (texte.includes('https://') && verifGroupe && yes) {
          ...
          if (superUser || verifAdmin || !verifZokAdmin) {
            console.log(`i'm taking no action`); return
          };
          ...
          if (action === 'remove') {
            ...
            await zk.sendMessage(origineMessage, {
              text: txt, mentions: [auteurMessage]
            }, {
              quoted: ms
            });
            try {
            ...
            if (warn >= warnlimit) {
              ...
              await zk.sendMessage(origineMessage, {
                text: kikmsg, mentions: [auteurMessage]
              }, {
                quoted: ms
              });
              await delay(500);
              ...
              await delay(500);
              await zk.sendMessage(origineMessage, {
                text: msg, mentions: [auteurMessage]
              }, {
                quoted: ms
              });
              await delay(500);
      ...
      try {
        ...
        if (botMsg || baileysMsg) {
          ...
          if (mtype === 'reactionMessage') {
            console.log('i\'m taking no action to the reaction'); return
          };
      ...
      if (verifCom) {
        ...
        const cd = evt.cm.find((zokou) => zokou.nomCom === (com)) || evt.cm.find((zokou) => zokou.alias.includes(com));
        if (cd) {
          try {
            ...
            if (!superUser) {
              ...
              if (req) {
                repondre("You are banned from bot commands"); return
              }
    ...
    zk.ev.on('group-participants.update',
      async (group) => {
        console.log(group);
        ...
        try {
          ...
          if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            ...
            if (group.author == metadata.owner || group.author == conf.NUMERO_OWNER + '@s.whatsapp.net' || group.author == decodeJid(zk.user.id) || group.author == group.participants[0]) {
              console.log('is super user so i\'m doing nothing'); return;
            };
            ...
            if (group.author == metadata.owner || group.author == conf.NUMERO_OWNER + '@s.whatsapp.net' || group.author == decodeJid(zk.user.id) || group.author == group.participants[0]) {
              console.log('is super user so taking no action'); return;
            };
...
╚─────────────────···▸▸`;
    ...
    zk.ev.on("creds.update",
      saveCreds);
    //fin événement authentification
    //
    /** ************* */
    //fonctions utiles
    zk.downloadAndSaveMediaMessage = async (message,
      filename = '',
      ...
      const stream = await (0,
        baileys_1.downloadContentFromMessage)(quoted,
        messageType);
